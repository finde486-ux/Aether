...
def create_dashboard(active_node: str, events: list, tension: dict) -> Layout:
    ...
    triad_table.add_row("ALPHA (Architect)", "WAITING" if active_node != "alpha" else "[bold]ACTIVE[/bold]")
    triad_table.add_row("OMEGA (Adversary)", "WAITING" if active_node != "omega" else "[bold]ACTIVE[/bold]")
    triad_table.add_row("SIGMA (Observer)", "WAITING" if active_node != "sigma" else "[bold]ACTIVE[/bold]")
    ...
    event_table.add_column("Time", size=12)
...
async def run_dashboard():
    ...
    config = {"configurable": {"thread_id": "aether_live_v1"}}

    with Live(refresh_per_second=4, screen=True) as live:
        # Run graph in background task
        graph_task = app.astream(state, config)
        ...
        while True:
            ...
            try:
                # Get next graph update if available
                # This is simplified for the dashboard view
                async for event in graph_task:
                    for node_name, output in event.items():
                         active_node = node_name
                    live.update(create_dashboard(active_node, events, tension))
                    await asyncio.sleep(0.5)

                # If graph finishes, stay on final state
                active_node = "FINISHED"
                live.update(create_dashboard(active_node, events, tension))
                await asyncio.sleep(1)

            except Exception as e:
                active_node = f"ERROR: {str(e)}"
                live.update(create_dashboard(active_node, events, tension))
                await asyncio.sleep(1)
